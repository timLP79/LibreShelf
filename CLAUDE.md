...
- [ ] **#22 -- Loan system (trimmed):** loan schema (`due_date`, `returned_at`) + transactional DB methods + checkout/return handlers wired to the existing book-detail scaffold + `/loans` page with `active | overdue` query-param filter + kiosk public anonymous browse. Favorites moved to deferred post-submission backlog on 2026-04-24 after a scope/timeline re-check (see backlog entry below for context). SSE live availability and patron holds deferred post-submission (unchanged from CLAUDE.md original). `fine_cents` column dropped from CP6 on 2026-04-24 -- will be re-added (shape TBD) when the overdue notice + fines feature is built, post-submission.
