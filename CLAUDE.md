...
### CP6 -- Loans + Kiosk + Dashboard -- complete (merged to main 2026-04-25 via PR #42)
...
- [x] **Kiosk public browse** (session 5, closed 2026-04-25). `/kiosk` and `/kiosk/books/:id` public, anonymous, separate `kiosk_layout.html` shell (no sidebar / nav / Sign Out / logout), `renderKioskTemplate` skips User/CSRFToken injection. Whole-card tap targets via `.kiosk-card-link` for touch terminals. No patron login gate in CP6. Bad ids redirect to `/kiosk` rather than render an error template (kept scope tight). Login page advertises the kiosk via a "Browse the catalog without an account" link under the Library Portal subtitle so visitors without credentials can discover it (cs408-go-stack-qqx, 2026-04-25).
...
- **Loan history view (raised post-CP6 on 2026-04-25):** CP6 ships only active and overdue views. A complete history (returned loans included) is not surfaced anywhere except the per-book panel on the staff book-detail page. Three audiences need it: (1) staff/admin should be able to pull a patron's full history (e.g. "this patron returns books late often"); (2) patrons should be able to see what they have read in the past, not just what they currently hold; (3) admins running an audit should be able to filter by date range. Design questions when un-deferred: route shape (`/patrons/:id/history` for staff vs `/my/history` for patrons -- separate routes preserve the privacy boundary built in CP6 around `/loans` vs `/my/loans`); whether to add a `?include=returned` flag on the existing endpoints or build a new pair; pagination strategy if loan rows grow past the all-in-DOM threshold; CSV export for the staff view (likely useful, slots into CP7 admin panel). DB layer impact: `GetActiveLoans` / `GetPatronActiveLoans` filter on `returned_at IS NULL`; new `GetLoanHistory(patronID, ...)` and `GetPatronFullHistory(patronID, ...)` queries needed, plus a count query for pagination. Existing `LoanListRow` struct is reusable -- add returned_at as an optional field. Templates: extend `loans.html` and `my_loans.html` with a "Show returned" toggle, OR build a dedicated history page that includes a "Returned on" column. Avoid leaking other-patron history to a patron-role user via the same route-group enforcement pattern as `/my/loans`.
...
- **Patron self-registration toggle (admin-controlled) (cs408-go-stack-o1x, raised 2026-04-25):** admin-only setting on `/admin` that gates a public `/register` route. Default OFF: only staff and admin can create patron accounts (current CP5 behavior, suitable for restricted environments like prisons or correctional facilities where inmates must not self-register). When ON: a public `/register` route is exposed and a "Create an account" link appears on the login page below Sign In (mirrors the kiosk link placement from cs408-go-stack-qqx). Design when un-deferred: new `settings (key TEXT PRIMARY KEY, value TEXT, updated_at TEXT, updated_by INTEGER FK users.id)` table cached at startup with write-through invalidate on update, default-deny on missing key; admin UI is a Bootstrap toggle with a confirm-modal on flip; `/register` short-circuits to 404 (or redirect to `/login`) when OFF so the route does not advertise itself; CSRF via the existing pre-session double-submit cookie pattern (same shape as login); reuses `CreatePatron` transactional flow but with user-chosen username (explicit uniqueness check rather than the `generateBaseUsername` collision-retry loop, which is for auto-generation); bcrypt cost matches existing; no email verification or admin approval queue in v1 (the toggle itself is the gate); auto-login on success + flash welcome to avoid a confusing extra step; flipping OFF does not delete already-self-registered accounts (they become indistinguishable from staff-created patrons). Rate limiting integration point flagged for CP7. Tests: setting read/write unit tests, handler tests for `/register` in both states (OFF returns 404, ON happy path, ON with duplicate username, ON with weak password), boundary test confirming `/register` is unreachable when OFF even with valid CSRF and form data.
