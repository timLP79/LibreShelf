...
// TestOpenLibraryLookup_FallsBackToWikipediaWhenOLDescriptionEmpty
// pins the integration: when OL returns metadata but no description
// (the Dune case), the handler hits Wikipedia, validates author match,
// and returns the Wikipedia extract in the JSON response with
// description_source="wikipedia". Wires both fakes through the
// existing URL variables.
func TestOpenLibraryLookup_FallsBackToWikipediaWhenOLDescriptionEmpty(t *testing.T) {
	router, dm := setupTestRouter(t)
	sess, _ := loginAs(t, dm, "staff_wiki_fallback", "staff")

	// Stub OL: returns full metadata, no description.
	...
	// Stub Wikipedia: direct title hit on "Dune" with Frank Herbert
	// in the extract.
	...
	req, _ := http.NewRequest("GET", "/api/openlibrary/isbn/9780441013593", nil)
	req.AddCookie(sess)
	rr := httptest.NewRecorder()
	router.ServeHTTP(rr, req)
	...
	if !strings.Contains(body, "Frank Herbert") {
		t.Errorf("response should contain Wikipedia extract; got %s", body)
	}
	if !strings.Contains(body, `"description_source":"wikipedia"`) {
		t.Errorf("response should mark description_source=wikipedia; got %s", body)
	}
}

// TestOpenLibraryLookup_OLDescriptionPreservedWhenLongEnough pins
// the OL-wins case: when OL's own description is already long enough
// (>= 80 chars), Wikipedia must NOT overwrite it, and description_source
// must be "openlibrary".
func TestOpenLibraryLookup_OLDescriptionPreservedWhenLongEnough(t *testing.T) {
	router, dm := setupTestRouter(t)
	sess, _ := loginAs(t, dm, "staff_ol_wins", "staff")

	const olBlurb = "When Elizabeth Bennet first meets eligible bachelor Fitzwilliam Darcy, she thinks him arrogant and conceited."
	...
	// Wikipedia stub should never be reached if the threshold logic
	// works, but wire one anyway so a regression that wrongly invokes
	// it is still visible in the assertions below.
	startFakeWikipedia(t, fakeWiki{
		summaryByTitle: map[string]string{
			"Pride_and_Prejudice": `{"type":"standard","extract":"WIKI_OVERRIDE_TEXT mentioning Jane Austen"}`,
		},
	})
	...
	body := rr.Body.String()
	if !strings.Contains(body, "Elizabeth Bennet") {
		t.Errorf("OL blurb should be preserved; got %s", body)
	}
	if strings.Contains(body, "WIKI_OVERRIDE_TEXT") {
		t.Errorf("Wikipedia must NOT overwrite a good OL description; got %s", body)
	}
	if !strings.Contains(body, `"description_source":"openlibrary"`) {
		t.Errorf("response should mark description_source=openlibrary; got %s", body)
	}
}
